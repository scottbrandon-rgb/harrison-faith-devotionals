...
const ogDataDir = path.join(__dirname, '..', 'netlify', 'shared');
...
// Also remove the old location if it exists (cleanup from previous version)
const oldOgDataFile = path.join(__dirname, '..', 'netlify', 'edge-functions', 'og-data.ts');
if (fs.existsSync(oldOgDataFile)) fs.unlinkSync(oldOgDataFile);
